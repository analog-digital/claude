...
**Granularity — no full-page shortcuts (this is where runs actually fail).**
- **Never judge fidelity from a single full-page screenshot.** A tall page downscaled to one image hides everything small — button fills, +/− icon states, individual dots, tile backgrounds, heading casing. Capture **each section zoomed in** enough that those are clearly legible, and compare at that zoom. A whole-page glance is not a section diff.
- **Tool failure does NOT relax the visual requirement.** If Pillow/PIL or any pixel-diff library is unavailable, do **not** fall back to "let me just view the full render." Do the per-section *zoomed* perceptual comparison by eye anyway. Missing tooling lowers precision (no % score); it never lowers coverage. Falling back to a single whole-page view is the #1 way this skill silently misses defects.
- **Emit a per-component ledger, or the gate is not run.** For each section, output a table: every button, badge, +/− control, dot, tile, eyebrow, and heading → its observed color/state vs the Figma value → `match` / `MISMATCH`. C3, C4, PAR5, and TF2 cannot be marked pass without this ledger. "I looked and it seemed fine" is not a result — name each component and its value.

**Orphan nodes (content authored outside the page frame).** If a section's page-frame render comes back empty or heading-only, do **NOT** conclude the section is fine, unbuilt, or matching. In poorly-framed files, section content (rows, cards, buttons) is often placed as loose canvas nodes *outside* the page frame. Call `get_metadata` on the whole page/canvas, find the section's real nodes by name (e.g. `FaqRow*`, `ExpandBtn*`, `*Card*`, `*Badge*`), and `get_screenshot` those nodes **directly** to compare. Never skip or pass a section because its frame render looked empty. (Field example: the FAQ heading was inside the page frame but the rows + expand buttons lived at x≈2360 *outside* it — the frame render showed only the title, which would have hidden that the teal +/− buttons should be orange.)
...
**Self-honesty check before you report (the run failed if any of these is true):** you judged a section from a full-page screenshot instead of a zoomed section view; you relaxed the visual check because a tool (PIL/pixel-diff) was missing; you marked C3/C4/PAR5/TF2 pass without the per-component ledger; or you passed/skipped a section whose Figma render looked empty without checking for orphan nodes outside the page frame. If any is true, you have not run the skill — go back and do the zoomed, per-component, per-section pass.
