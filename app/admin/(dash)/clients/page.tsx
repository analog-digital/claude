      ...
      <div
        className={`mt-4 flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm ${
          stripeOn
            ...
            : "border-[var(--color-surface)] bg-white text-[var(--color-text)]"
        }`}
      >
        <span className={`h-2 w-2 rounded-full ${stripeOn ? "bg-[var(--color-primary)]" : "bg-amber-400"}`} />
        {stripeOn ? (
          ...
          <span>
            ...
            <code className="mx-1 rounded bg-[var(--color-surface)] px-1">STRIPE_SECRET_KEY</code> to enable.
          </span>
        )}
      </div>
            ...
            <tbody className="divide-y divide-[var(--color-surface)]">
              {clients.map((c) => (
                <tr key={c.id} className="align-top">
                  <td className="whitespace-nowrap px-4 py-3 text-xs text-[var(--color-text)]">{c.created_at}</td>
                  <td className="px-4 py-3">
                    <div className="font-medium">{c.name}</div>
                    <a href={`mailto:${c.email}`} className="block text-xs text-[var(--color-primary)] hover:underline">
                      {c.email}
                    </a>
                    {c.phone && <div className="text-xs text-[var(--color-text)]">{c.phone}</div>}
                  </td>
                  <td className="px-4 py-3 text-xs">
                    <div>{c.plan ?? "—"}</div>
                    <div className="text-[var(--color-text)]">{c.box_size ?? ""}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-xs text-[var(--color-text)]">
                    {c.stripe_customer_id ? (
                      <span className="rounded bg-[var(--color-primary-light)] px-2 py-0.5 text-[var(--color-primary)]">Stripe linked</span>
                    ) : (
                      "Manual"
                    )}
                  </td>
